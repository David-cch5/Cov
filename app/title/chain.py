...
def walk_chain_of_title(session, covid: int, tract_no: int = 1) -> dict:
...
def _walk_via_recorder_portal(session, covid: int, covenant, parcel, registry,
    ...
    def _lookup_doc(doc_number: str) -> dict | None:
        def _call():
            with recorder_context() as context:
                return publicsearch.search_by_document_number(context, base_url, doc_number)
        return run_with_job_queue(_call, job_type="title_chain_doc_lookup", county_fips=covenant.county_fips,
                                   covid=covid, payload={"base_url": base_url, "doc_number": doc_number})

    anchor = _lookup_doc(covenant.recording_instrument)
    if anchor is None:
        return [{"ambiguous_split": True, "review_reason": f"could not re-locate the covenant's own recorded "
                 f"instrument ({covenant.recording_instrument!r}) to establish a lot/block anchor"}]
    anchor_lots = _parse_lots(anchor.get("LOT"))
    anchor_block = anchor.get("BLOCK")

    def _matches_anchor(row: dict) -> bool:
        row_lots = _parse_lots(row.get("LOT"))
    ...
    seed_query = " ".join(situs.split()[:2]) if situs else covenant.declarant_raw
    for row in _search(seed_query) + [anchor]:
    ...
    for _hop in range(MAX_HOPS):
        conveyance_candidates = []
        for doc_num, row in candidate_pool.items():
            if doc_num in consumed:
                continue
            doc_type = (row.get("DOC TYPE") or "").strip().upper()
            ...
            if doc_type not in CONVEYANCE_DOC_TYPES:
                continue
            if _names_match(row.get("GRANTOR", ""), current_holder):
                conveyance_candidates.append((recorded, row))
