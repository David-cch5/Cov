"""Build the play/drag formation map for a covenant, from the database.

Generalised from the hand-built covid 4440 platting map, and moved out of
scripts/ so a pipeline stage can call it -- the whole point of the exercise was
that this should not be a script somebody remembers to run per covenant.

Output goes to _artifacts/, an app-owned directory, and is served by
app/web (route /covenant/<covid>/map) rather than published anywhere. Publishing
is an outward-facing act and stays a deliberate human one.
...
selected date -- read from parcel.formed_date, which stands on a recorded
...
is drawn grey throughout. An animation that quietly filled those in would look
better and assert something nobody read.
...
ARTIFACT_DIR = os.path.join(PROJECT_ROOT, "_artifacts")
...
def artifact_path(covid: int) -> str:
    """Where a covenant's map lives. One stable path per covid, so regenerating
    replaces rather than accumulating, and app/web can link to it without being
    told where it went."""
    return os.path.join(ARTIFACT_DIR, f"covid{covid}_formation_map.html")


def build(session, covid: int, simplify_ft: float = 10.0) -> dict:
    """Generate the map and write it. Returns what it found and what it wrote.

    Never raises for a covenant that simply has nothing to draw -- no geometry, no
    census, no formation date are all ordinary states, not errors, and a caller
    running this across the whole portfolio should not have to catch them. It
    reports `written=None` with a reason instead.
    """
    cov = _covenant(session, covid)
    tracts = _tracts(session, covid, simplify_ft)
    if not tracts:
        return {"covid": covid, "written": None,
                "reason": "no tract with both geometry and a parcel census"}
    ...
    if not dated:
        return {"covid": covid, "written": None, "parcels": parcel_total,
                "reason": (f"none of {parcel_total} parcels has a recorded formation date -- "
                           f"nothing to animate. Raw abstract-survey tracts have no formation "
                           f"event to read; see app/gis/formation.py")}
    ...
    facts = "".join(
        ...
        for k, v in [
            ...
            ("Formation dates", str(len(set(dated)))),
        ])
    ...
    for t in tracts:
        ...
        sections += SECTION.format(
            tract_no=t["tract_no"], view=VIEW, t0=t0,
    ...
    out = artifact_path(covid)
    with open(out, "w", encoding="utf-8") as f:
        f.write(html)
    return {"covid": covid, "written": out, "bytes": len(html), "tracts": len(tracts),
            "parcels": parcel_total, "undated": undated_total,
            "formation_dates": len(set(dated)), "from": t0, "to": t1, "reason": None}


def eligible_covids(session) -> list[int]:
    """Every covenant with geometry and a census -- the ones a map can be built
    for at all, regardless of where they sit in review."""
    return [r[0] for r in session.execute(text("""
        SELECT DISTINCT t.covid
          FROM tract t
